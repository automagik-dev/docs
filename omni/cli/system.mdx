...
```bash
omni completions [shell]   # Shell type: bash, zsh, fish
```
...
View and stream system logs with module and level filtering.

```bash
omni logs [level]                          # Filter by level: debug, info, warn, error
omni logs --modules <modules>              # Comma-separated module filter
omni logs --limit <n>                      # Limit results (default: 100)
omni logs --process [service]              # Stream PM2 process logs (default: api)
omni logs --process api --follow           # Stream live logs
```
