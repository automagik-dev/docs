...
<Warning>
Route priority matters. When multiple routes match (per-user, per-chat, per-instance), the **most specific scope wins**, not the highest priority number. Priority only breaks ties within the same scope level. See [Routes concept page](/omni/concepts/routes) for the full resolution order.
</Warning>
...
<Tip>
Always run `omni routes test` after creating or updating routes to verify the resolution matches your intent. This is especially useful when combining per-user and per-chat routes on the same instance.
</Tip>
...
<Warning>
`omni keys revoke` disables a key immediately — any services using it will lose access. `omni keys delete` is permanent and cannot be undone. Prefer `revoke` for temporary access removal.
</Warning>
