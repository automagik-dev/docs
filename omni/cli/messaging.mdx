...
| `--thread-id <id>` | Thread/topic ID (e.g., Telegram forum topic) |
...
<Tip>
Use `--presence composing` before sending text and `--presence recording` before sending voice notes to make bot interactions feel more natural.
</Tip>
...
<Warning>
Presence indicators are channel-specific. WhatsApp supports `composing`, `recording`, and `paused`. Other channels may ignore unsupported presence types silently.
</Warning>
...
omni chats update <chat-id> [options]              # Update a chat
omni chats delete <chat-id>                        # Delete a chat
```
...
<Note>
`messages delete` triggers a "delete for everyone" on WhatsApp. The message must have been sent recently — WhatsApp enforces a time window after which deletion is no longer possible.
</Note>
