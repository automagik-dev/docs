...
<CardGroup cols={2}>
  <Card title="Genie" icon="hat-wizard">
    Team inbox integration with Automagik Genie orchestration framework.
  </Card>
  <Card title="Claude Code" icon="terminal">
    Anthropic Claude Code CLI agent — spawns a session per conversation.
  </Card>
  <Card title="A2A / AG-UI" icon="arrows-left-right">
    Google A2A and CopilotKit AG-UI agent-to-agent protocols.
  </Card>
  <Card title="Agno / OpenClaw" icon="robot">
    Agno platform agents and OpenClaw open-source agents via WebSocket.
  </Card>
  <Card title="Webhook" icon="webhook">
    POST event payloads to any URL — the simplest integration path.
  </Card>
</CardGroup>
...
# Create a provider (use --schema for type)
...
<Warning>
Provider API keys are stored in the Omni database. Use scoped keys with minimal permissions when possible, and rotate keys regularly. Never commit API keys to version control.
</Warning>

## Agents

Agents are the entities that receive and process messages.
