---
title: "Signing-Identity Rotation"
sidebarTitle: "Key Rotation"
icon: "key"
description: "When and how to rotate the cosign certificate identity that operators verify against — and what stays unchanged in routine releases."
---

<Note>
  Release signing for `@automagik/genie` is **cosign keyless only**. There is no long-lived private key, no hardware-backed offline key, no public-key fingerprint to pin. "Rotation" here means rotating the **certificate-identity pin** (workflow-path@ref, OIDC issuer, and provenance source-uri) that operators cross-check across three independent channels.
</Note>
