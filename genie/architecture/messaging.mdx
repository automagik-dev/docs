...
<Warning>
**The lifecycle-event subjects below are historical.** Genie once emitted `genie.tool.*`, `genie.msg.*`, `genie.user.*` and `genie.agent.*` from an in-process hook handler chain. That chain was deleted in the `skills-everywhere` wish — there is no hook command and no lifecycle hook of any kind in a Genie release, so nothing publishes those subjects any more. See the [release notes](/genie/release-notes) for what is gone and what to do instead.
</Warning>

The NATS path that survives is the **Omni bridge**: `genie omni serve` is a foreground runner that subscribes to the Omni subjects configured by `OMNI_NATS_URL` and drains inbound messages into the global approval queue and inbox (`~/.genie/genie.db`). It is the only optional daemon Genie ships; everything else forks and exits.

| Behavior | Detail |
|----------|--------|
| Publisher | The Omni server, not Genie |
| Subscriber | `genie omni serve` (foreground, opt-in) |
| Sink | Approval queue + inbox in the global `genie.db` |
| Inspect | `genie omni status`, `genie omni inbox` (both offline) |
