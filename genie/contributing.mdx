...
```text
src/
├── genie.ts                          # CLI entry point (commander), 15 registrations
├── lib/                              # Core modules
...
registration, no hook definitions, and no editor-specific integration. If you are extending Genie, you are almost
certainly writing a **skill** (below) or a **saved workflow**, not a plugin.

### The Orca plugin payload

```text
plugins/genie/
├── orca-plugin.json        # The manifest — must sit at the install tree's ROOT
├── package.json            # Versioned alongside the root package.json
├── references/             # The public Orca boundary contract
└── src/                    # Handlers over `orca orchestration … --json`
```
