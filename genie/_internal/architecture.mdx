...
This document describes the internal architecture of the Genie CLI, including the unified serve model, service topology, agent lifecycle, and data flow.
...
│
├── 4. Scheduler daemon     Claims and fires triggers from PG
│     ├── Event router      Routes PG NOTIFY events to team members
│     └── Inbox watcher     Polls native inboxes, auto-spawns offline leads
│
└── 5. Service registry     In-memory PID tracker for graceful shutdown
```
...
<Warning>
**The hook system is gone.** Genie once ran an in-process handler chain, blocking and non-blocking — branch guard, git-freeze guard, identity injection, Omni approval, audit context, and SessionStart context/freshness injection. All of it was deleted in the `skills-everywhere` wish: there is no hook command, no handler chain, and no lifecycle hook of any kind in a Genie release. The controls those handlers approximated now live where they are actually enforceable — server-side branch protection on `main`, worktree isolation, sandbox permissions, and operator policy carried in briefs and `AGENTS.md`. Passages below that describe hook handlers as live (identity injection during spawn, `runtime-emit-*` event emission) are historical. See the [release notes](/genie/release-notes) for what is gone and what to do instead.
</Warning>
