...
Genie ships a single, dark-only color palette — petrol surfaces, mint-green text, alarms reserved for genuine danger. One source of truth, three consumers (TUI, desktop app, tmux). No backward-compat aliases.

## Why this palette

The design reference is a dark terminal: black-petrol surfaces, mint-green monospace text, alarms reserved for genuine danger. Institutional cool greys carry the chrome; a warm amber is held in reserve. Red means a real emergency, not "RAM at 51%". This forces calm defaults and makes attention states actually attentive.
...
├── palette.ts                primitive hex values (petrol/mint)
...
### Surfaces — institutional cool

| Token | Hex | Use |
|-------|-----|-----|
| `bg` | `#0a1d2a` | App background — terminal petrol |
...
| `textMuted` | `#5e6e74` | Tertiary / muted grey |

### Accent — terminal text
...
`beige` (`#d4c5a9`), `mutedGrey` (`#5e6e74`), `warmAmber` (`#d4a574`).

### Semantic aliases

`tokens.ts` re-exports the palette under intent-based names — `surface`, `surfaceRaised`, `danger`, `dangerStrong`, `attention`, `muted`, `warm`, `beige`. Prefer these in new code; they survive palette rebalancing without churning import sites.

## How to add a new color

1. Add the primitive to `packages/genie-tokens/palette.ts` with a one-line comment describing what the color is for.
...
- Light mode — the palette is dark-only by design.
