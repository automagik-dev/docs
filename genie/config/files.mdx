...
description: "config.json schema, PostgreSQL state, environment variables, and auto-approve configuration"
---
...
Genie does not write into any agent's settings file. The one place it puts content inside an agent's configuration is the agent's **skill home**, and it gets there through the [skills.sh](https://skills.sh) channel — run by the bootstrap script on first install, and by `genie update` on every update:
...
<Note>
`agentDirs` is a post-install discovery scan of your home directory, not a fixed table — it names every directory the skills CLI actually wrote. `genie doctor` reads this record to report drift, and `genie uninstall` reads it (and re-verifies `dirDigests`) to decide what it may safely delete. Genie never modifies `~/.claude/settings.json`.
</Note>
