...
`standalone` is the default, including when the setting is absent. Switching to `orca` probes the shipped plugin payload and a compatible Orca runtime before it switches; after that, Genie refuses local lifecycle reads and writes. Switching back imports no Orca state.

<Note>
  There is no per-agent integration step. Genie's skills reach Codex, Claude Code, and every other agent skills.sh supports through the skills channel that `genie update` installs — see [Installation](/genie/installation#skills-for-any-agent).
</Note>
