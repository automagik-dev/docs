...
Starting with the release that ships the skills.sh channel (Genie 5.2609xx and later), `genie update` performs this global install itself, so you do not need to run the command by hand. On earlier versions, run the command above once and again after each update. The `npx skills add` command is also how people who want Genie's skills without the Genie CLI install them, and how you get a project-scoped copy inside a single repository.
