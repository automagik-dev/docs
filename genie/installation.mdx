...
Genie's skills are published through [skills.sh](https://skills.sh), so any agent that reads skills can use them — Claude Code, Codex, Cursor, and the rest. To install them machine-wide, for every agent detected on the machine, with no prompts:
...
- `--all` answers the agent picker for you and installs into every detected agent instead of prompting.
- `--copy` writes plain files instead of symlinks — add it when your agent or sandbox does not follow links.

If you use the Genie CLI you do not need to run this yourself: `genie update` already performs the global, all-agents install as part of delivering a new version. The `npx skills add` command is for people who want Genie's skills without the Genie CLI, or who want a project-scoped copy inside a single repository.
