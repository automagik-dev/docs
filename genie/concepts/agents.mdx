...
Each spawn creates a tmux pane and registers the agent in the PostgreSQL `agents` table. See the [Spawn & Lifecycle CLI reference](/genie/cli/spawn) for all spawn options and the [agent management commands](/genie/cli/infrastructure) for listing and inspecting agents.
