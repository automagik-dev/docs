...
Genie tracks execution state separately from the wish document:

```
.genie/state/<slug>.json
```
...
The state file is shared across worktrees (via `git rev-parse --git-common-dir`), so multiple agents working in different worktrees see the same progress.
...
| Execution state | `.genie/state/<slug>.json` | Progress tracking |
