...
| Option | Description |
|--------|-------------|
| `--since <duration>` | Time window to scan (e.g., `24h`, `7d`) |
| `--json` | Output as JSON |
| `--breakdown` | Show per-session cost breakdown |

```bash
genie events scan
genie events scan --since 7d --breakdown
```
...
|--------|-------------|
| `--json` | Output as JSON |
| `--verbose` | Show detailed per-spec output |
...
| Option | Description |
|--------|-------------|
| `--team <name>` | Team to evaluate against |
| `--json` | Output as JSON |

```bash
genie qa check specs/messaging.yaml
genie qa check specs/messaging.yaml --team auth-rewrite --json
```
